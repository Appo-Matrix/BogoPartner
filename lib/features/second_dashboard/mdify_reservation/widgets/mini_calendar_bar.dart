        ...
        return Center(
          child: InkWell(
            ...
            child: Container(
              ...
              height: 36,
              decoration: BoxDecoration(
                color: isSel ? PAppColors.primary : Colors.transparent,
                borderRadius: BorderRadius.circular(16),
              ),
              alignment: Alignment.center,
              child: Text(
...
extension _Inject on Column {
  Column _injectPickers(Widget months, Widget years) {
    ...
    final mi = kids.indexWhere(
      (w) => w is SizedBox && (w).child is _MonthsProxy,
    );
    final yi = kids.indexWhere(
      (w) => w is SizedBox && (w).child is _YearsProxy,
    );
